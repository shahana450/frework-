...
export default function InsightsPage() {
  ...
  async function loadInsights(bid: string) {
    ...
    // GST payable estimate â€” sum output vs input GST from account names
  ...
  const fmtShort = (n: number) => n >= 100000 ? `â‚¹${(n / 100000).toFixed(1)}L` : n >= 1000 ? `â‚¹${(n / 1000).toFixed(1)}K` : `â‚¹${Math.round(n)}`;
    ...
    <div style={{ minHeight: "100vh", background: "#070C1A", color: "#EDE8DC", fontFamily: "system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "2rem" }}>
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <p style={{ margin: 0, color: "rgba(237,232,220,0.4)", fontSize: "0.82rem" }}>{bizName} â€” Current Financial Year</p>
        </div>

        {loading ? (
          <div style={{ textAlign: "center", padding: "5rem", color: "rgba(237,232,220,0.3)" }}>Analysing your financialsâ€¦</div>
        ) : (
          <>
            {/* KPI Strip */}
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: "0.75rem", marginBottom: "1.5rem" }}>
              {[
                { label: "Revenue", value: totalRevenue, color: "#4ade80", icon: "ðŸ“ˆ" },
                { label: "Expenses", value: totalExpenses, color: "#f87171", icon: "ðŸ“‰" },
                { label: "Net Profit", value: netProfit, color: netProfit >= 0 ? "#C9A84C" : "#f87171", icon: "ðŸ’°" },
                { label: "GST Payable", value: gstPayable, color: "#60a5fa", icon: "ðŸ›ï¸" },
                { label: "Cash & Bank", value: cashBalance, color: "rgba(237,232,220,0.8)", icon: "ðŸ¦" },
              ].map(k => (
            ...
            <div style={{ background: parseFloat(profitMargin) >= 0 ? "rgba(74,222,128,0.06)" : "rgba(248,113,113,0.06)", border: `1px solid ${parseFloat(profitMargin) >= 0 ? "rgba(74,222,128,0.2)" : "rgba(248,113,113,0.2)"}`, borderRadius: 12, padding: "1rem 1.5rem", marginBottom: "1.5rem", display: "flex", alignItems: "center", gap: "2rem" }}>
              ...
              <div style={{ flex: 1, fontSize: "0.85rem", color: "rgba(237,232,220,0.5)", lineHeight: 1.6 }}>
                {parseFloat(profitMargin) >= 20
                  ...
                  : "Operating at a loss. Immediate action needed â€” cut costs or increase revenue."}
              </div>
                      ...
                      return (
                        <div key={m.month} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
                          ...
                          <div style={{ display: "flex", gap: 2, alignItems: "flex-end", height: 140 }}>
                            <div title={`Revenue: â‚¹${fmt(m.revenue)}`} style={{ width: 14, height: revH, background: "linear-gradient(180deg,#4ade80,#22c55e)", borderRadius: "3px 3px 0 0", transition: "height 0.4s" }} />
                            <div title={`Expenses: â‚¹${fmt(m.expenses)}`} style={{ width: 14, height: expH, background: "linear-gradient(180deg,#f87171,#ef4444)", borderRadius: "3px 3px 0 0", transition: "height 0.4s" }} />
                          </div>
                        ...
                        <div key={i} style={{ marginBottom: "0.75rem" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.25rem" }}>
                            ...
                            <div style={{ display: "flex", gap: "0.5rem", flexShrink: 0 }}>
                              <span style={{ fontSize: "0.75rem", fontVariantNumeric: "tabular-nums" }}>â‚¹{fmt(d.amount)}</span>
            ...
            <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(237,232,220,0.07)", borderRadius: 12, padding: "1.25rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.65rem" }}>
                {[
                  gstPayable > 0 && { icon: "ðŸ›ï¸", text: `GST payable estimated at â‚¹${fmt(gstPayable)}. File GSTR-3B before the 20th.`, color: "#60a5fa", href: "/finance/gst" },
                  netProfit < 0 && { icon: "âš ï¸", text: "Business is operating at a loss this year. Review your top expenses and pricing.", color: "#f87171", href: "/finance/virtual-ca" },
                  totalRevenue === 0 && { icon: "ðŸ“ˆ", text: "No sales recorded yet. Create your first sales invoice to start tracking revenue.", color: "#C9A84C", href: "/finance/sales/new" },
                  totalExpenses > totalRevenue * 0.8 && totalRevenue > 0 && { icon: "ðŸ“Š", text: `Expenses are ${((totalExpenses / totalRevenue) * 100).toFixed(0)}% of revenue. Industry benchmark is typically 60â€“70%.`, color: "#fb923c", href: "/finance/reports" },
                  gstPayable > 0 && { icon: "âœ…", text: "Ensure GST ITC is claimed on all purchase bills with valid GSTIN.", color: "#4ade80", href: "/finance/virtual-ca" },
                  { icon: "ðŸ“‹", text: "Keep all invoices uploaded and AI-reviewed for accurate books and easy ITR filing.", color: "rgba(237,232,220,0.5)", href: "/finance/upload" },
                  ...
                  <Link key={i} href={alert.href} style={{ display: "flex", alignItems: "center", gap: "0.85rem", padding: "0.65rem 0.85rem", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(237,232,220,0.06)", borderRadius: 8, textDecoration: "none" }}>
                    ...
                    <span style={{ fontSize: "0.75rem", color: "#C9A84C", flexShrink: 0 }}>â†’</span>
                  </Link>
                ))}
              </div>
            </div>
