...
export default function CreditNotePage() {
  ...
  // Recent credit notes
  const [recent, setRecent] = useState<{ id: string; date: string; entry_no: string; narration: string; total_debit: number }[]>([]);
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const [bizRes, fyRes, contactsRes, accountsRes, recentRes] = await Promise.all([
        ...
        supabase.from("fw_fin_contacts").select("id,name,state").eq("business_id", saved).eq("type", "customer").order("name"),
        supabase.from("fw_fin_chart_of_accounts").select("id,code,name,type").eq("business_id", saved).eq("is_group", false).order("code"),
        supabase.from("fw_fin_journals").select("id,date,entry_no,narration,total_debit").eq("business_id", saved).eq("type", "credit_note").order("date", { ascending: false }).limit(8),
      ]);
      ...
      const salesAcc = accs.find(a => a.name.toLowerCase().includes("sales") || a.name.toLowerCase().includes("revenue"));
      const debtorAcc = accs.find(a => a.name.toLowerCase().includes("debtor") || a.name.toLowerCase().includes("receivable"));
      if (salesAcc) setSalesAccountId(salesAcc.id);
      if (debtorAcc) setDebtorAccountId(debtorAcc.id);
    });
  }, []);
  ...
  async function handlePost() {
    if (!bizId || !fyId || !amt || !salesAccountId || !debtorAccountId) { setError("Fill all required fields"); return; }
    setSaving(true); setError(""); setSuccess("");

    const { count } = await supabase.from("fw_fin_journals").select("id", { count: "exact" }).eq("business_id", bizId).eq("type", "credit_note");
    const entryNo = `CN-${String((count ?? 0) + 1).padStart(4, "0")}`;
    ...
    const lines: { account_id: string; dr_amount: number; cr_amount: number; narration: string }[] = [
      { account_id: salesAccountId, dr_amount: amt, cr_amount: 0, narration: `Credit Note â€” ${reason.replace("_", " ")} to ${contactName}` },
    ];

    // GST reversal â€” find GST accounts
    ...
    if (gstAmt > 0) {
      if (!isInterState && cgstAcc && sgstAcc) {
        lines.push({ account_id: cgstAcc.id, dr_amount: cgst, cr_amount: 0, narration: "CGST reversed" });
        lines.push({ account_id: sgstAcc.id, dr_amount: sgst, cr_amount: 0, narration: "SGST reversed" });
      } else if (isInterState && igstAcc) {
        lines.push({ account_id: igstAcc.id, dr_amount: igst, cr_amount: 0, narration: "IGST reversed" });
      }
    }

    lines.push({ account_id: debtorAccountId, dr_amount: 0, cr_amount: totalAmt, narration: `AR reduced â€” CN ${entryNo}` });
    ...
    const { data: journal, error: jErr } = await supabase.from("fw_fin_journals").insert({
      ...
      narration: narration || `Credit Note to ${contactName} â€” ${reason.replace("_", " ")}`,
    ...
    const { data: rec } = await supabase.from("fw_fin_journals").select("id,date,entry_no,narration,total_debit").eq("business_id", bizId).eq("type", "credit_note").order("date", { ascending: false }).limit(8);
    setRecent(rec ?? []);
  }
    ...
    <div style={{ minHeight: "100vh", background: "#070C1A", color: "#EDE8DC", fontFamily: "system-ui,sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: "1.5rem", maxWidth: 1000, margin: "0 auto", padding: "2rem" }}>
        <div>
          ...
          <p style={{ margin: "0 0 1.5rem", color: "rgba(237,232,220,0.4)", fontSize: "0.82rem" }}>Issue a credit note to a customer â€” reduces their outstanding (AR) balance</p>
          ...
          <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(237,232,220,0.07)", borderRadius: 12, padding: "1.25rem" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
              <div>
                ...
                <select value={reason} onChange={e => setReason(e.target.value)} style={inputStyle}>
                  ...
                  <option value="discount_post_sale">Post-Sale Discount</option>
                  <option value="damaged_goods">Damaged / Defective Goods</option>
                  <option value="short_supply">Short Supply</option>
                  <option value="other">Other</option>
                </select>
              </div>
              <div><label style={labelStyle}>Against Invoice No</label><input value={refInvoice} onChange={e => setRefInvoice(e.target.value)} placeholder="e.g. INV-0001" style={inputStyle} /></div>
            </div>
            ...
            {amt > 0 && (
              <div style={{ background: "rgba(237,232,220,0.02)", border: "1px solid rgba(237,232,220,0.06)", borderRadius: 8, padding: "0.75rem 1rem", marginBottom: "1rem", fontSize: "0.8rem" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}><span>Base Amount</span><span style={{ fontVariantNumeric: "tabular-nums" }}>â‚¹{fmt(amt)}</span></div>
                {!isInterState && cgst > 0 && <>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}><span>CGST @ {parseFloat(gstRate) / 2}%</span><span style={{ fontVariantNumeric: "tabular-nums" }}>â‚¹{fmt(cgst)}</span></div>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}><span>SGST @ {parseFloat(gstRate) / 2}%</span><span style={{ fontVariantNumeric: "tabular-nums" }}>â‚¹{fmt(sgst)}</span></div>
                </>}
                {isInterState && igst > 0 && <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}><span>IGST @ {gstRate}%</span><span style={{ fontVariantNumeric: "tabular-nums" }}>â‚¹{fmt(igst)}</span></div>}
                <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 800, borderTop: "1px solid rgba(237,232,220,0.08)", paddingTop: 6, marginTop: 6, color: "#f87171" }}>
                  <span>Total Credit Note Value</span><span style={{ fontVariantNumeric: "tabular-nums" }}>â‚¹{fmt(totalAmt)}</span>
                </div>
              </div>
            )}
            ...
            <div style={{ marginBottom: "1rem" }}>
              ...
              <textarea value={narration} onChange={e => setNarration(e.target.value)} placeholder="e.g. Sales return against INV-0001 â€” damaged goods" rows={2} style={{ ...inputStyle, resize: "vertical" }} />
            </div>
            ...
            <button onClick={handlePost} disabled={saving || !amt} style={{ background: "#f87171", border: "none", color: "#070C1A", padding: "10px 24px", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: "0.9rem", opacity: (!amt || saving) ? 0.5 : 1 }}>
              {saving ? "Postingâ€¦" : "Post Credit Note"}
            </button>
          </div>
        </div>
        ...
        {/* Recent */}
        <div>
          <div style={{ fontSize: "0.72rem", color: "rgba(237,232,220,0.35)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "0.75rem", marginTop: "3.8rem" }}>Recent Credit Notes</div>
            ...
            {recent.map(r => (
              <div key={r.id} style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(237,232,220,0.06)", borderRadius: 8, padding: "0.7rem 0.85rem" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.2rem" }}>
                  <span style={{ fontSize: "0.72rem", fontFamily: "monospace", color: "rgba(248,113,113,0.6)" }}>{r.entry_no}</span>
                ...
                <div style={{ textAlign: "right", marginTop: "0.25rem", fontSize: "0.82rem", fontWeight: 700, color: "#f87171" }}>â‚¹{fmt(r.total_debit)}</div>
              </div>
            ))}
