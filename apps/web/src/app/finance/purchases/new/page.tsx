          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
            <div>
              ...
              <select value={paymentMode} onChange={e => setPaymentMode(e.target.value as "credit" | "bank" | "cash")} style={inputStyle}>
                <option value="credit">Credit (Creditor â€” pay later)</option>
              ...
              <tbody>
                {lines.map((line, i) => (
                  <tr key={i} style={{ borderTop: "1px solid rgba(237,232,220,0.04)" }}>
                    ...
                    <td style={{ padding: "0.4rem 0.75rem" }}>
                      {lines.length > 1 && <button onClick={() => setLines(p => p.filter((_, idx) => idx !== i))} style={{ background: "none", border: "none", color: "rgba(248,113,113,0.5)", cursor: "pointer" }}>âœ•</button>}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(237,232,220,0.07)", borderRadius: 12, padding: "1.25rem", minWidth: 240 }}>
            {[
              ...
              <div key={r.label} style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.5rem" }}>
                ...
                <span style={{ fontSize: "0.8rem", fontVariantNumeric: "tabular-nums" }}>â‚¹{fmt(r.value)}</span>
              </div>
            ))}
            ...
            {totalGST > 0 && <div style={{ fontSize: "0.7rem", color: "#4ade80", marginTop: "0.35rem", textAlign: "right" }}>ITC: â‚¹{fmt(totalGST)}</div>}
          </div>
        </div>
      </div>
    </div>
