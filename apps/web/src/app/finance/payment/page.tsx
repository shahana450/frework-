...
export default function PaymentPage() {
  ...
  async function handleSave(mode: "draft" | "post") {
    ...
    setSuccess(`${mode === "post" ? "Posted" : "Saved"}: ${entryNo} â€” â‚¹${parseFloat(amount).toLocaleString("en-IN")}`);
          ...
          <div style={{ display: "flex", gap: "0.75rem", marginBottom: "1.5rem" }}>
            {(["receipt", "payment"] as const).map(t => (
              <button key={t} onClick={() => setPaymentType(t)} style={{
                ...
                {t === "receipt" ? "ðŸ’° Customer Receipt" : "ðŸ’³ Vendor Payment"}
              </button>
            ))}
          </div>
          ...
          <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(237,232,220,0.07)", borderRadius: 12, padding: "1.25rem" }}>
            ...
            {parseFloat(amount) > 0 && (
              <div style={{ background: "rgba(237,232,220,0.03)", border: "1px solid rgba(237,232,220,0.06)", borderRadius: 8, padding: "0.75rem 1rem", marginBottom: "1rem", fontSize: "0.8rem" }}>
                ...
                {paymentType === "receipt" ? (
                  <>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}><span>Dr Bank / Cash</span><span style={{ color: "#f87171", fontVariantNumeric: "tabular-nums" }}>â‚¹{parseFloat(amount).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span></div>
                    <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ paddingLeft: "1rem" }}>Cr Debtors (AR)</span><span style={{ color: "#4ade80", fontVariantNumeric: "tabular-nums" }}>â‚¹{parseFloat(amount).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span></div>
                  </>
                ) : (
                  <>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}><span>Dr Creditors (AP)</span><span style={{ color: "#f87171", fontVariantNumeric: "tabular-nums" }}>â‚¹{parseFloat(amount).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span></div>
                    <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ paddingLeft: "1rem" }}>Cr Bank / Cash</span><span style={{ color: "#4ade80", fontVariantNumeric: "tabular-nums" }}>â‚¹{parseFloat(amount).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span></div>
                  </>
                )}
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: "0.75rem" }}>
              ...
              <button onClick={() => handleSave("post")} disabled={saving} style={{ background: paymentType === "receipt" ? "#4ade80" : "#f87171", border: "none", color: "#070C1A", padding: "9px 22px", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: "0.88rem" }}>
                {saving ? "Savingâ€¦" : `Post ${paymentType === "receipt" ? "Receipt" : "Payment"}`}
              </button>
            </div>
          </div>
        </div>
            ...
            {recent.map(r => (
              <div key={r.id} style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(237,232,220,0.06)", borderRadius: 8, padding: "0.7rem 0.85rem" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: "0.25rem" }}>
                  ...
                  <span style={{ fontSize: "0.82rem", fontVariantNumeric: "tabular-nums", color: r.type === "receipt" ? "#4ade80" : "#f87171" }}>â‚¹{fmt(r.type === "receipt" ? r.total_credit : r.total_debit)}</span>
                </div>
              </div>
            ))}
