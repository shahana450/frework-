          ...
          {!selectedAcc ? (
            ...
            <>
              <div style={{ marginBottom: "1.5rem" }}>
                ...
                <div style={{ fontSize: "0.78rem", color: "rgba(237,232,220,0.4)" }}>Account Code: {selectedAcc.code} Â· Type: {selectedAcc.type}</div>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "0.75rem", marginBottom: "1.5rem" }}>
                {[
                  ...
                  <div key={k.label} style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(237,232,220,0.07)", borderRadius: 10, padding: "0.85rem 1rem" }}>
                    ...
                    <div style={{ fontSize: "1.2rem", fontWeight: 800, color: k.color, fontVariantNumeric: "tabular-nums" }}>â‚¹{fmt(k.value)}</div>
                  </div>
                ))}
              </div>

              {loading ? (
                <div style={{ textAlign: "center", padding: "3rem", color: "rgba(237,232,220,0.3)" }}>Loading ledgerâ€¦</div>
              ) : entries.length === 0 ? (
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ background: "rgba(255,255,255,0.02)" }}>
                        {["Date", "Entry No", "Narration", "Debit (â‚¹)", "Credit (â‚¹)", "Balance"].map(h => (
                          <th key={h} style={{ padding: "0.55rem 0.85rem", textAlign: ["Debit (â‚¹)", "Credit (â‚¹)", "Balance"].includes(h) ? "right" : "left", fontSize: "0.68rem", color: "rgba(237,232,220,0.4)", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      <tr style={{ background: "rgba(201,168,76,0.04)", borderBottom: "1px solid rgba(237,232,220,0.06)" }}>
                        ...
                        <td style={{ padding: "0.5rem 0.85rem", textAlign: "right", fontSize: "0.8rem", color: "rgba(237,232,220,0.5)", fontVariantNumeric: "tabular-nums" }}>
                          {openingType === "Dr" && openingBal > 0 ? fmt(openingBal) : "â€”"}
                        </td>
                        <td style={{ padding: "0.5rem 0.85rem", textAlign: "right", fontSize: "0.8rem", color: "rgba(237,232,220,0.5)", fontVariantNumeric: "tabular-nums" }}>
                          {openingType === "Cr" && openingBal > 0 ? fmt(openingBal) : "â€”"}
                        </td>
