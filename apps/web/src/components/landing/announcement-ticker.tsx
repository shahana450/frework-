  ...
  return (
    ...
    >
      ...
      <div className="sm:hidden">
        {/* Row 1: LIVE badge + headline + dismiss */}
        <div className="flex items-center gap-2.5 px-3 pt-2 pb-1">
          <span className="flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-black tracking-widest flex-shrink-0"
          ...
          <div className="flex-1 overflow-hidden">
            <AnimatePresence mode="wait">
              <motion.div key={item?.id}
                ...
                <a href={item?.url && item.url !== "#" ? item.url : undefined}
          ...
          <button onClick={() => setDismissed(true)} className="p-1 rounded flex-shrink-0" style={{ color: "rgba(255,255,255,0.35)" }}>
            <X size={13} />
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 px-3 pb-2.5">
          ...
          <div className="flex items-center gap-1 mr-auto">
            {headlines.slice(0, Math.min(headlines.length, 5)).map((_, i) => (
              <button key={i}
                ...
                style={{ width: i === current ? 14 : 5, height: 5, background: i === current ? "#B8903A" : "rgba(184,144,58,0.3)" }} />
            ))}
          </div>
          ...
          {loggedIn ? (
            <Link href="/dashboard"
              ...
              style={{ background: "linear-gradient(135deg,#E8C97A,#B8903A)", color: "#1A1208", boxShadow: "0 2px 8px rgba(184,144,58,0.5)" }}>
      ...
      <div className="hidden sm:flex max-w-7xl mx-auto px-8 items-center gap-4 h-14">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={item?.id}
              ...
              <a href={item?.url && item.url !== "#" ? item.url : undefined}
                target={item?.url && item.url !== "#" ? "_blank" : undefined}
                rel="noopener noreferrer"
                className="text-[13px] font-medium leading-snug truncate transition-colors hover:text-white"
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          {headlines.slice(0, Math.min(headlines.length, 6)).map((_, i) => (
            <button key={i}
              onClick={() => { setCurrent(i); if (intervalRef.current) clearInterval(intervalRef.current); }}
              className="rounded-full transition-all duration-300"
              style={{ width: i === current ? 16 : 6, height: 6, background: i === current ? "#B8903A" : "rgba(184,144,58,0.3)" }} />
          ))}
        </div>
        ...
        <div className="flex items-center gap-2.5 flex-shrink-0">
          <Link href="/pricing"
            className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-[12px] font-black tracking-wide transition-all hover:scale-[1.05]"
            style={{ background: "linear-gradient(135deg,rgba(59,130,246,0.25),rgba(30,64,175,0.35))", color: "#93C5FD", border: "1px solid rgba(59,130,246,0.35)", boxShadow: "0 0 12px rgba(59,130,246,0.15)" }}>
          ...
          {loggedIn ? (
            <Link href="/dashboard"
              ...
              style={{ background: "linear-gradient(135deg,#E8C97A,#B8903A)", color: "#1A1208", boxShadow: "0 2px 10px rgba(184,144,58,0.45)" }}>
              <LayoutDashboard size={13} /> Dashboard
            </Link>
          ) : (
            <>
              <Link href="/login"
                className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-[13px] font-bold transition-all hover:bg-white/10"
